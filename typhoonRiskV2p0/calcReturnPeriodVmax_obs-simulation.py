...
for iKey in totalDict.keys():
    ...
    mean = VmaxSim.mean()
    std  = VmaxSim.std()
    print(locSim1,scaleSim1,mean,std) 
    ...
    cSim3, locSim3, scaleSim3 = st.weibull_min.fit(VmaxSim) 
    cObs3, locObs3, scaleObs3 = st.weibull_min.fit(VmaxObs) 
    ...
    VmaxSimGumbel   = st.gumbel_r.ppf(arg,loc=mean,scale=std)
    VmaxSimFrechet  = st.invweibull.ppf(arg,c=1,loc=mean,scale=std)
    VmaxSimWeibull  = st.weibull_min.ppf(arg,c=1,loc=mean,scale=std) 
    VmaxSimPearson3 = st.pearson3.ppf(arg,skew=1,loc=mean,scale=std) 
