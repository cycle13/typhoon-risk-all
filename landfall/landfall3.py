...
class typhoonLandfall:
    ...
    def getProfilePoints(self,province):
        '''
        获取省份轮廓点
    ...
    def getLandfallPoints(self,tyData,province):
        ...
        if province == 'Guangdong': 
            ...
            allProvinces = ['Hainan','Guangxi','Guangdong','Hongkong','Macao','Fujian','Zhejiang','Shanghai','Jiangsu','Shangdong','Liaoning','Hebei','Tianjin']
        else:
            pass
        ...
        for iProvince in allProvinces:
            iProfilePoints = self.getProfilePoints(iProvince)
            iRegionPoly = Polygon(list(zip(*iProfilePoints))).simplify(0.05)
        ...
        if province != 'China': 
            ...
            allRegionPolyUnion = allRegionPolyDict['Hainan']
            for iProvince in allProvinces:
                iRegionPoly = allRegionPolyDict[iProvince]
                allRegionPolyUnion = allRegionPolyUnion.union(iRegionPoly)
    ...
    def getDistricts(self,province):
        ...
        if province != 'Guangdong':
            for info, shape in zip(m.chn_info, m.chn):
                if info['NAME_1'] == province and Polygon(shape).area > 0.001:
...
if __name__ == '__main__': 
    ...
    ldTyInfo, count = landfall.getLfTyInfo(data,'Guangxi')
    print(count) 
